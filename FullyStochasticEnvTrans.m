...
for k=1:sim
clear s_h s_w s_d i_w i_d i_h t_d r_h r_w r_d t N_h N_d N_w j x v_w v_d v_t
...
v_w = zeros(4,prealloc_size); v_d = zeros(4,prealloc_size); v_t = zeros(4,prealloc_size);
...
count=0;
  ...
  ev8=((beta_dd*s_d(g,j)*i_d(g,j)+beta_wd*s_d(g,j)*i_w(g,j))/N_d + s_d(g, j)*(rho_d*v_d(g, j)+rho_d*v_w(g, j))/dose)*dt+ev7;% infection of sd by id or iw
  ev9=(beta_td*s_d(g,j)*t_d(g,j)/N_d + rho_d*s_d(g, j)*v_t(g, j)/dose)*dt+ev8;% infection of sd by td
  ...
  ev18=((beta_th*s_h(g,j)*t_d(g,j)+beta_hh*s_h(g,j)*i_h(g,j))/N_h + rho_h*s_h(g, j)*v_t(g, j)/dose)*dt+ev17;% infection of human
  ...
  ev24=omega_d*i_d(g, j)*dt/dose+ev23; % shed infectious dose v_d
  ev25=omega_t*t_d(g, j)*dt/dose+ev24; % shed infectious dose v_t
  ev26=eta_w(g)*v_w(g, j)*dt/dose+ev25; % decay infectious dose v_w
  ev27=eta_d(g)*v_d(g, j)*dt/dose+ev26; % decay infectious dose v_d
  ev28=eta_t(g)*v_t(g, j)*dt/dose+ev27; % decay infectious dose v_t
...
v_w = v_w(g, 1:j); v_d = v_d(g, 1:j); v_t = v_t(g, 1:j); 
t = t(1, 1:j) + 90*(g - 1) + 60;
