%%%%%%%%%%%%%%%%%%%%%%%%%% 
% CTMC Model Environmental Transmission Hybrid Model
% %%%%%%%%%%%%%%%%%%%%%%%%
clear all;
close all;
dbstop if error
...
R0w=0.3; R0d=0.3; R0t=0.3; R0h=0.3;
...
for k=1:sim
clear s_h s_w s_d i_w i_d i_h t_d r_h r_w r_d t N_h N_d N_w j x
...
beta_ww = zeros(1, prealloc_size); x = zeros(4, prealloc_size); 
...
count=0;
  ...
  ev4=dt*(beta_ww(j)*s_w(g,j)*i_w(g,j)/N_w + rho*s_w(g, j)*(1 - exp(- alpha*omega*x(j))))+ev3; % infection of wild bird
  ...
  x(g, j + 1) = x(g, j)*exp(-eta(g)*dt) + i_w(g, j)*(1 - exp(-eta(g)*dt))/eta(g);
  j=j+1;
